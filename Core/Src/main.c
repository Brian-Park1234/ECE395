...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    printf("WS2812 Demo\n");
//CHANGE THE NUMBER OF LEDs currently 200
    ws2812_init(&ws2812, &htim4, TIM_CHANNEL_1, 200);
...
void SystemClock_Config(void)
{
    ...
    RCC_OscInitStruct.PLL.PLLN = 128;   // 2 MHz × 128 = 256 MHz
    RCC_OscInitStruct.PLL.PLLP = RCC_PLLP_DIV4;   // 256 / 4 = 64 MHz
    RCC_OscInitStruct.PLL.PLLQ = 5;     // For USB (not needed but valid)
